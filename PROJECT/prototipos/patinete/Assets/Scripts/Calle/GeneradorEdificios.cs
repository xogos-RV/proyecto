...
public class GeneradorEdificios : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Inicializar los pools para cada prefab de edificios
        foreach (GameObject prefab in buildingPrefabs)
        {
            objectPool.CreatePool(prefab, 10); // Crear un pool de 10 objetos para cada prefab
        }

        // Inicializar los pools para cada prefab de árboles/papeleras
        foreach (GameObject prefab in arboles_papeleras)
        {
            objectPool.CreatePool(prefab, 10); // Crear un pool de 10 objetos para cada prefab
        }
    }
    ...
    void GenerarElementoEnCalle(ref float ultimaPosicionZ, float posicionX, List<GameObject> calle, int mirror, ref int ultimoPrefabUsado, ref bool ultimoEspacioVacio)
    {
        ...
        if (generarEdificio)
        {
            ...
            nuevoObjeto.transform.position = posicionObjeto;
            nuevoObjeto.transform.rotation = prefabSeleccionado.transform.rotation;
    ...
    void DestruirObjetosAntiguosDeLista(List<GameObject> lista, float distanciaDestruccion)
    {
        for (int i = lista.Count - 1; i >= 0; i--)
        {
            if (lista[i].transform.position.z < liegreCam.position.z - distanciaDestruccion)
            {
                ...
                objectPool.ReturnObject(lista[i]);
                lista.RemoveAt(i);
            }
        }
    }
